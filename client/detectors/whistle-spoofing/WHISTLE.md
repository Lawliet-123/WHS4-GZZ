...
## 반복 관측에서 whistle_rpc (2026-09-23)

```
python main.py --session rpc_watch_001 --only whistle_rpc --watch 120 --interval 5
```

단발 실행은 후크 로그를 처음부터 끝까지 읽는다. 반복 관측에서 그렇게 하면
위반이 한 번 찍힌 뒤로 모든 바퀴가 계속 DETECTED 가 되고, 이전 게임 실행의
위반까지 섞인다 — 후크가 로그를 덮어쓰지 않고 이어 쓰기 때문이다.

그래서 반복 관측에서는 `begin_watch()` 가 시작 시점의 파일 끝을 기준점으로 잡고,
바퀴마다 **새로 쓰인 줄만** 읽는다. 호출 수는 후크가 3초마다 남기는 누적 카운터의
증가분이라 최대 3초 늦게 잡힐 수 있다.

이 구간에 도발 호출이 0건이면 **ERROR 가 아니라 NORMAL** 이다(단발 모드와 다름).
휘파람을 안 불었을 뿐이고, 후크가 붙어 있는지는 시작할 때 확인한다.
`evidence.meta.window_calls` 로 "안 불어서 0" 과 "불었는데 정상" 을 가른다.
