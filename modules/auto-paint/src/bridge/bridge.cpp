...
namespace
{
    ...
    constexpr std::uintptr_t GObjectsRva = 0x097DE5A0;
    constexpr std::uintptr_t GNamesRva = 0x09983780;
    ...
    constexpr std::uintptr_t OffFPropertyElementSize = 0x34;
    constexpr std::uintptr_t OffFPropertyOffset = 0x44;
    constexpr std::uintptr_t OffFStructPropertyStruct = 0x70;

    constexpr std::uint32_t BridgeStartMagic = 0x3153434D; // "MCS1"
    constexpr std::uint32_t BridgeStartSize = 128;
    constexpr std::uint32_t BridgeStartProtocol = 1;
...
#pragma pack(push, 1)
    struct BridgeStartBlockV1
    {
        ...
        std::uint8_t InstanceId[16];
        std::uint8_t Token[32];
        std::uint8_t BridgeSha256[32];
        std::uint32_t Reserved0;
        std::uint32_t State;
        std::uint32_t Port;
        std::uint32_t BootstrapProtocol;
        std::uint32_t Win32Error;
        std::uint32_t WinsockError;
        std::uint32_t Reserved1;
        std::uint32_t Reserved2;
    };
#pragma pack(pop)
    static_assert(sizeof(BridgeStartBlockV1) == BridgeStartSize, "BridgeStartBlockV1 layout mismatch");
    static_assert(offsetof(BridgeStartBlockV1, State) == 100, "BridgeStartBlockV1 State offset mismatch");
    static_assert(offsetof(BridgeStartBlockV1, Port) == 104, "BridgeStartBlockV1 Port offset mismatch");
    static_assert(offsetof(BridgeStartBlockV1, BootstrapProtocol) == 108, "BridgeStartBlockV1 protocol offset mismatch");

    HMODULE g_module = nullptr;
    std::atomic<bool> g_running{true};
    std::atomic<bool> g_bootstrap_started{false};
    std::string g_bootstrap_instance_id;
    std::string g_bootstrap_token;
    std::string g_bootstrap_bridge_hash;
    ...
    auto bytes_to_hex(const std::uint8_t* bytes, std::size_t size) -> std::string
    {
    ...
    auto compact_mesh_response_metadata(const std::string& metadata) -> std::string
    {
        ...
        for (const char* key : {
                 ...
                 "fill_region_count",
                 "skip_region_count",
                 "source_direct_assignments",
                 "replay_non_direct_skipped",
                 "replay_strokes_total",
                 "server_batch_limit",
    ...
    struct FNameResolver
    {
        ...
        auto resolve(std::uint32_t id) -> std::string
        {
            if (detected)
            {
                return entry(id, table_offset, style);
            }
    ...
    struct Reflection
    {
        ...
        auto init(std::string& failure) -> bool
        {
            ...
            if (!module.base)
            {
                failure = "main_module_unavailable";
                return false;
            }
            auto valid_guobject_outer = [&](std::uintptr_t outer) {
                const auto array = outer + 0x10;
                const auto chunks = safe_read<std::uintptr_t>(array);
                const auto max_elements = safe_read<std::int32_t>(array + 0x10, 0);
                const auto num_elements = safe_read<std::int32_t>(array + 0x14, 0);
                const auto max_chunks = safe_read<std::int32_t>(array + 0x18, 0);
                const auto num_chunks = safe_read<std::int32_t>(array + 0x1C, 0);
                return chunks && safe_read<std::uintptr_t>(chunks) &&
                       num_elements > 0 && num_elements <= 10000000 &&
                       max_elements >= num_elements && max_elements <= 16000000 &&
                       num_chunks > 0 && num_chunks <= max_chunks && max_chunks <= 64;
            };
            auto try_fname_pool = [&](std::uintptr_t candidate) {
                if (!candidate)
                {
                    return false;
                }
                names.pool = candidate;
                names.detect();
                if (names.detected && names.resolve(0) == "None")
                {
                    fname_pool = candidate;
                    return true;
                }
                return false;
            };

            const auto static_outer = module.base + GObjectsRva - 0x10;
            if (valid_guobject_outer(static_outer))
            {
                guobject_array = static_outer;
            }
            ...
            if (!guobject_array)
            {
                ...
                const auto raw_candidate = gu_ref + 7 + rel;
                if (valid_guobject_outer(raw_candidate))
                {
                    guobject_array = raw_candidate;
                }
                else if (valid_guobject_outer(raw_candidate - 0x10))
                {
                    guobject_array = raw_candidate - 0x10;
                }
                else
                {
                    failure = "guobject_candidate_invalid";
                    return false;
                }
            }

            const auto static_names = module.base + GNamesRva;
            if (try_fname_pool(static_names) || try_fname_pool(safe_read<std::uintptr_t>(static_names)))
            {
                return true;
            }
            ...
            for (const auto& [sig, mask] : patterns)
            {
                ...
                const auto candidate = ref + 7 + fname_rel;
                if (try_fname_pool(candidate) || try_fname_pool(safe_read<std::uintptr_t>(candidate)))
                {
                    return true;
                }
            }
        ...
        {
            const auto array = guobject_array + 0x10;
            const auto chunks = safe_read<std::uintptr_t>(array);
            const auto num_elements = safe_read<std::int32_t>(array + 0x14, 0);
            const auto num_chunks = safe_read<std::int32_t>(array + 0x1C, 0);
            if (!chunks || num_elements <= 0 || num_elements > 10000000 ||
                num_chunks <= 0 || num_chunks > 64)
            {
                return;
            }
            int remaining = num_elements;
            for (int ci = 0; ci < num_chunks && remaining > 0; ++ci)
            {
                ...
                const int chunk_count = std::min(65536, remaining);
                for (int wi = 0; wi < chunk_count; ++wi)
                {
    ...
    auto sdk_context_metadata(Reflection& ref, const SdkContext& ctx) -> std::string
    {
        return "\"sdk_version\":\"runtime_dynamic_reflection_min\"" +
               ...
               ",\"runtime_paintable_offset\":\"dynamic_component_scan\"" +
    ...
    struct SdkNativeFrontSampleResult
    {
        ...
        std::string failure{};
        std::uintptr_t mesh{0};
        std::vector<std::uintptr_t> meshes_to_hide{};
        std::uintptr_t hit_test_function{0};
        std::string sampling_backend{"unset"};
        bool keep_occluded_projected_samples{false};
        sdk::ESceneCaptureSource capture_source{sdk::ESceneCaptureSource::BaseColor};
        std::string capture_source_name{"base_color"};
        int capture_settle_ms{48};
    ...
    struct SdkFrontCaptureResult
    {
        ...
        int hide_component_requested{0};
        int hide_component_succeeded{0};
        bool capture_scene_called{false};
        int capture_scene_calls{0};
        int capture_settle_ms{0};
        std::string capture_source{"base_color"};
    ...
    struct MeshFirstProfileWindingAnalysis
    {
        bool ok{false};
        bool inverted{false};
        int outward{0};
    ...
    auto mesh_first_analyze_profile_winding(const MeshFirstProfile& profile) -> MeshFirstProfileWindingAnalysis
    {
        MeshFirstProfileWindingAnalysis out{};
        const int expected_triangles = profile.index_count / 3;
        if (!profile.ok || profile.vertices.empty() || expected_triangles <= 0 ||
            static_cast<int>(profile.indices.size()) < profile.index_count ||
            static_cast<int>(profile.triangles.size()) != expected_triangles)
        {
            out.failure = "profile_geometry_unavailable";
            return out;
        }

        sdk::FVector centroid{};
        for (const auto& vertex : profile.vertices)
        {
            centroid = sdk_vec_add(centroid, vertex.position);
        }
        centroid = sdk_vec_mul(centroid, 1.0 / static_cast<double>(profile.vertices.size()));

        for (int triangle_index = 0; triangle_index < expected_triangles; ++triangle_index)
        {
            const std::size_t base = static_cast<std::size_t>(triangle_index) * 3;
            const int i0 = profile.indices[base];
            const int i1 = profile.indices[base + 1];
            const int i2 = profile.indices[base + 2];
            if (i0 < 0 || i1 < 0 || i2 < 0 ||
            ...
            const auto& p0 = profile.vertices[static_cast<std::size_t>(i0)].position;
            const auto& p1 = profile.vertices[static_cast<std::size_t>(i1)].position;
            const auto& p2 = profile.vertices[static_cast<std::size_t>(i2)].position;
            auto normal = sdk_vec_normalize(profile.triangles[static_cast<std::size_t>(triangle_index)].local_normal);
            if (sdk_vec_len(normal) <= 0.000001)
            {
                normal = sdk_vec_normalize(sdk_vec_cross(sdk_vec_sub(p1, p0), sdk_vec_sub(p2, p0)));
            }
            const auto center = sdk_vec_mul(sdk_vec_add(sdk_vec_add(p0, p1), p2), 1.0 / 3.0);
            const auto radial = sdk_vec_sub(center, centroid);
            const double alignment = sdk_vec_dot(normal, radial);
            if (!std::isfinite(alignment) || sdk_vec_len(normal) <= 0.000001 || sdk_vec_len(radial) <= 0.000001 ||
                std::abs(alignment) <= 0.000001)
            {
                ++out.ambiguous;
            }
        ...
        const int classified = out.outward + out.inward;
        if (classified <= 0)
        {
            out.failure = "profile_winding_unclassified";
            return out;
        }
        out.inverted = out.inward > out.outward;
        out.confidence = static_cast<double>(std::max(out.inward, out.outward)) /
                         static_cast<double>(classified);
        const int minimum_classified = std::max(32, expected_triangles / 2);
        out.ok = classified >= minimum_classified && out.confidence >= 0.60;
        out.failure = out.ok ? "ok" : "profile_winding_low_confidence";
        return out;
    }
    ...
    struct MeshFirstPlanSample
    {
        ...
        bool direct_capture_color{false};
        bool unsafe{false};
    };
    ...
    auto mesh_first_reconstruct_profile_pose_triangles(const MeshFirstProfile& profile,
                                                       const std::vector<sdk::FVector>& component_positions,
                                                       const std::vector<sdk::FVector>& world_positions) -> MeshFirstRuntimeTriangleCache
    {
        MeshFirstRuntimeTriangleCache out{};
        if (!profile.ok || profile.index_count <= 0 || profile.index_count % 3 != 0)
        {
            out.failure = "profile_pose_reconstruction_invalid_profile";
            return out;
        }
        if (static_cast<int>(profile.indices.size()) < profile.index_count ||
            static_cast<int>(profile.vertices.size()) != profile.vertex_count ||
            component_positions.size() != profile.vertices.size() ||
            world_positions.size() != profile.vertices.size())
        {
            out.failure = "profile_pose_reconstruction_vertex_count_mismatch";
            return out;
        }

        const int triangle_count = profile.index_count / 3;
        std::vector<MeshFirstRuntimeTriangle> triangles{};
        triangles.resize(static_cast<std::size_t>(triangle_count));
        for (int triangle_index = 0; triangle_index < triangle_count; ++triangle_index)
        {
            auto& triangle = triangles[static_cast<std::size_t>(triangle_index)];
            ...
            for (int vertex_slot = 0; vertex_slot < 3; ++vertex_slot)
            {
                ...
                if (vertex_index < 0 || vertex_index >= profile.vertex_count)
                {
                    out.failure = "profile_pose_reconstruction_invalid_index";
                    return out;
                }
                const auto vertex = static_cast<std::size_t>(vertex_index);
                triangle.local[vertex_slot] = component_positions[vertex];
                triangle.world[vertex_slot] = world_positions[vertex];
                triangle.uv[vertex_slot].X = profile.vertices[vertex].u;
                triangle.uv[vertex_slot].Y = profile.vertices[vertex].v;
                if (!mesh_first_finite_vector(triangle.local[vertex_slot]) ||
                    !mesh_first_finite_vector(triangle.world[vertex_slot]) ||
                    !mesh_first_finite_uv(triangle.uv[vertex_slot]))
                {
                    out.failure = "profile_pose_reconstruction_non_finite_vertex";
                    return out;
                }
            }
        }
                                             ...
                                             const char* reason) -> MeshFirstRuntimePaintWarmup
    {
        ...
        const auto initialize_function = ref.find_function(ctx.component, "InitializePaint");
        out.initialize_available = initialize_function != 0;
        out.initialize_called = false;
        out.initialize_ok = false;
        const bool selected_mesh_is_initialized =
            initialized_before.process_ok &&
            initialized_before.value &&
            out.initialized_mesh_before == mesh;
        if (!out.initialize_available)
        {
            out.initialize_skip_reason = "initialize_paint_unavailable";
        }
        else if (!live_uobject(mesh))
        {
            out.initialize_skip_reason = "selected_mesh_unavailable";
        }
        else if (selected_mesh_is_initialized)
        {
            out.initialize_skip_reason = "already_initialized_for_selected_mesh";
        }
        else
        {
            out.initialize_called = true;
            sdk::RuntimePaintableComponent_InitializePaint params{};
            params.MeshComponent = reinterpret_cast<void*>(mesh);
            std::string failure{};
            const bool process_ok = process_event(ctx.component,
                                                  initialize_function,
                                                  reinterpret_cast<std::uint8_t*>(&params),
                                                  failure);
            out.initialize_ok = process_ok && params.ReturnValue;
            out.initialize_skip_reason = out.initialize_ok
                ? "initialized_selected_mesh"
                : (process_ok ? "initialize_paint_returned_false" : "initialize_paint_process_event_failed");
            if (!out.initialize_ok)
            {
                if (!out.failure.empty())
                {
                    out.failure += ";";
                }
                out.failure += process_ok
                    ? "initialize_paint:returned_false"
                    : "initialize_paint:" + failure;
            }
        }
    ...
    auto mesh_first_generate_plan_samples_from_runtime_cache(const MeshFirstProfile* profile,
                                                             ...
                                                             const sdk::FVector& camera_location,
                                                             const sdk::FVector& camera_direction,
                                                             bool invert_camera_facing_winding,
                                                             char region_axis,
                                                             ...
                                                             std::string& failure) -> bool
    {
        ...
        for (std::size_t tri = 0; tri < static_cast<std::size_t>(expected_triangles); ++tri)
        {
            ...
            const double raw_camera_facing = sdk_vec_dot(world_normal, camera_direction);
            const double camera_facing = invert_camera_facing_winding
                                             ? -raw_camera_facing
                                             : raw_camera_facing;
    ...
    struct MeshFirstScreenHitRefineResult
    {
        bool ok{false};
        std::uintptr_t function{0};
        int seed_projected{0};
        int bbox_min_x{0};
        int bbox_min_y{0};
        int bbox_max_x{0};
        int bbox_max_y{0};
        int requested_cell_px{0};
        int effective_cell_px{0};
        int max_attempts{0};
        int attempts{0};
        int process_ok{0};
        int hits{0};
        int invalid_uv{0};
        int invalid_world{0};
        int projection_rejected{0};
        int duplicate_uv{0};
        int uv_triangle_lookup_resolved{0};
        int uv_triangle_lookup_unresolved{0};
        double uv_triangle_world_delta_avg_cm{0.0};
        double uv_triangle_world_delta_max_cm{0.0};
        double projection_delta_avg_px{0.0};
        double projection_delta_max_px{0.0};
        std::string failure{"not_run"};
        std::vector<MeshFirstPlanSample> samples{};
    };

    auto mesh_first_lookup_profile_triangle_at_uv(const MeshFirstProfile& profile,
                                                   const std::vector<MeshFirstRuntimeTriangle>& runtime_triangles,
                                                   double u,
                                                   double v,
                                                   const sdk::FVector& hit_world,
                                                   int& triangle_index,
                                                   double& barycentric_a,
                                                   double& barycentric_b,
                                                   double& barycentric_c,
                                                   double& world_delta_cm) -> bool
    {
        triangle_index = -1;
        barycentric_a = 1.0;
        barycentric_b = 0.0;
        barycentric_c = 0.0;
        world_delta_cm = std::numeric_limits<double>::infinity();
        if (!profile.ok ||
            runtime_triangles.empty() ||
            runtime_triangles.size() != profile.triangles.size())
        {
            return false;
        }

        // HitUV can lie a fraction outside a triangle because it is quantized by
        // the game's hit-test path.  A small tolerance keeps seam pixels while
        // world-space distance disambiguates overlapping UV islands.
        constexpr double kUvTolerance = 0.0025;
        for (std::size_t tri_index = 0; tri_index < runtime_triangles.size(); ++tri_index)
        {
            const auto& triangle = runtime_triangles[tri_index];
            const double x0 = triangle.uv[0].X;
            const double y0 = triangle.uv[0].Y;
            const double x1 = triangle.uv[1].X;
            const double y1 = triangle.uv[1].Y;
            const double x2 = triangle.uv[2].X;
            const double y2 = triangle.uv[2].Y;
            const double denominator = (y1 - y2) * (x0 - x2) + (x2 - x1) * (y0 - y2);
            if (!std::isfinite(denominator) || std::abs(denominator) <= 1.0e-14)
            {
                continue;
            }
            const double a = ((y1 - y2) * (u - x2) + (x2 - x1) * (v - y2)) / denominator;
            const double b = ((y2 - y0) * (u - x2) + (x0 - x2) * (v - y2)) / denominator;
            const double c = 1.0 - a - b;
            if (!std::isfinite(a) || !std::isfinite(b) || !std::isfinite(c) ||
                a < -kUvTolerance || b < -kUvTolerance || c < -kUvTolerance ||
                a > 1.0 + kUvTolerance || b > 1.0 + kUvTolerance || c > 1.0 + kUvTolerance)
            {
                continue;
            }
            const auto predicted_world = sdk_vec_add(
                sdk_vec_add(sdk_vec_mul(triangle.world[0], a), sdk_vec_mul(triangle.world[1], b)),
                sdk_vec_mul(triangle.world[2], c));
            const double delta = sdk_vec_len(sdk_vec_sub(predicted_world, hit_world));
            if (!std::isfinite(delta) || delta >= world_delta_cm)
            {
                continue;
            }
            triangle_index = static_cast<int>(tri_index);
            barycentric_a = a;
            barycentric_b = b;
            barycentric_c = c;
            world_delta_cm = delta;
        }
        return triangle_index >= 0;
    }

    auto mesh_first_refine_plan_with_uncached_screen_hits(Reflection& ref,
                                                           const SdkContext& ctx,
                                                           std::uintptr_t mesh,
                                                           const SdkViewportInfo& viewport,
                                                           const sdk::FTransform& component_to_world,
                                                           const MeshFirstProfile* profile,
                                                           const std::vector<MeshFirstRuntimeTriangle>* runtime_triangles,
                                                           const std::vector<MeshFirstPlanSample>& seed_samples,
                                                           int texture_size,
                                                           int requested_cell_px,
                                                           int max_attempts,
                                                           int min_hits,
                                                           double max_projection_delta_px) -> MeshFirstScreenHitRefineResult
    {
        MeshFirstScreenHitRefineResult out{};
        out.requested_cell_px = requested_cell_px;
        out.max_attempts = max_attempts;
        if (!live_uobject(ctx.component) || !live_uobject(ctx.controller) || !live_uobject(mesh) ||
            viewport.width <= 0 || viewport.height <= 0)
        {
            out.failure = "screen_hittest_context_unavailable";
            return out;
        }
        out.function = ref.find_function(ctx.component, "HitTestAtScreenPosition");
        if (!out.function)
        {
            out.failure = "screen_hittest_function_unavailable";
            return out;
        }

        double min_x = static_cast<double>(viewport.width);
        double min_y = static_cast<double>(viewport.height);
        double max_x = 0.0;
        double max_y = 0.0;
        bool bounds_initialized = false;
        for (const auto& sample : seed_samples)
        {
            if (!sample.source_candidate)
            {
                continue;
            ...
            double x = 0.0;
            double y = 0.0;
            if (!sdk_project_world_to_screen(ref, ctx, sample.world_position, x, y) ||
                !std::isfinite(x) || !std::isfinite(y))
            {
                continue;
            }
            if (x < 0.0 || y < 0.0 || x >= static_cast<double>(viewport.width) || y >= static_cast<double>(viewport.height))
            {
                continue;
            }
            ++out.seed_projected;
        ...
        if (!bounds_initialized || out.seed_projected <= 0)
        {
            out.failure = "screen_hittest_seed_projection_empty";
            return out;
        }

        const int base_cell = std::max(1, requested_cell_px);
        const int padding = std::max(12, base_cell * 4);
        out.bbox_min_x = std::max(0, static_cast<int>(std::floor(min_x)) - padding);
        out.bbox_min_y = std::max(0, static_cast<int>(std::floor(min_y)) - padding);
        out.bbox_max_x = std::min(viewport.width - 1, static_cast<int>(std::ceil(max_x)) + padding);
        out.bbox_max_y = std::min(viewport.height - 1, static_cast<int>(std::ceil(max_y)) + padding);
        if (out.bbox_max_x < out.bbox_min_x || out.bbox_max_y < out.bbox_min_y)
        {
            out.failure = "screen_hittest_bbox_invalid";
            return out;
        }

        int effective_cell = base_cell;
        const auto estimate_attempts = [&](int cell) -> std::int64_t {
            const auto cols = (out.bbox_max_x - out.bbox_min_x) / std::max(1, cell) + 1;
            const auto rows = (out.bbox_max_y - out.bbox_min_y) / std::max(1, cell) + 1;
            return static_cast<std::int64_t>(cols) * static_cast<std::int64_t>(rows);
        };
        while (effective_cell < 64 && estimate_attempts(effective_cell) > std::max(1, max_attempts))
        {
            ++effective_cell;
        }
        out.effective_cell_px = effective_cell;

        const double texture_extent = static_cast<double>(std::max(1, texture_size - 1));
        std::unordered_set<std::uint64_t> seen_uv{};
        seen_uv.reserve(static_cast<std::size_t>(std::max(1, max_attempts)));
        out.samples.reserve(static_cast<std::size_t>(std::max(1, std::min(max_attempts, 20000))));
        double projection_delta_sum = 0.0;
        double uv_triangle_world_delta_sum = 0.0;
        for (int y = out.bbox_min_y; y <= out.bbox_max_y && out.attempts < max_attempts; y += effective_cell)
        {
            for (int x = out.bbox_min_x; x <= out.bbox_max_x && out.attempts < max_attempts; x += effective_cell)
            {
                ++out.attempts;
                sdk::RuntimePaintableComponent_HitTestAtScreenPosition params{};
                params.MeshComponent = reinterpret_cast<void*>(mesh);
                params.ScreenPosition = sdk::FVector2D{static_cast<double>(x), static_cast<double>(y)};
                params.PlayerController = reinterpret_cast<void*>(ctx.controller);
                params.bUseCachedTriangles = false;
                std::string failure{};
                if (!process_event(ctx.component, out.function, reinterpret_cast<std::uint8_t*>(&params), failure))
                {
                    continue;
                }
                ++out.process_ok;
                if (!params.ReturnValue.bSuccess)
                {
                    continue;
                }
                const auto& hit_uv = params.ReturnValue.HitUV;
                const auto& hit_world = params.ReturnValue.HitWorldPosition;
                if (!std::isfinite(hit_uv.X) || !std::isfinite(hit_uv.Y) ||
                    hit_uv.X < -0.001 || hit_uv.X > 1.001 || hit_uv.Y < -0.001 || hit_uv.Y > 1.001)
                {
                    ++out.invalid_uv;
                    continue;
                }
                if (!mesh_first_finite_vector(hit_world))
                {
                    ++out.invalid_world;
                    continue;
                }
                double hit_x = 0.0;
                double hit_y = 0.0;
                if (!sdk_project_world_to_screen(ref, ctx, hit_world, hit_x, hit_y))
                {
                    ++out.projection_rejected;
                    continue;
                }
                const double dx = hit_x - static_cast<double>(x);
                const double dy = hit_y - static_cast<double>(y);
                const double delta = std::sqrt(dx * dx + dy * dy);
                if (!std::isfinite(delta) || delta > max_projection_delta_px)
                {
                    ++out.projection_rejected;
                    continue;
                }
                projection_delta_sum += delta;
                out.projection_delta_max_px = std::max(out.projection_delta_max_px, delta);

                const auto u_pixel = static_cast<std::uint32_t>(std::clamp<int>(
                    static_cast<int>(std::lround(clamp01(hit_uv.X) * texture_extent)),
                    0,
                    std::max(0, texture_size - 1)));
                const auto v_pixel = static_cast<std::uint32_t>(std::clamp<int>(
                    static_cast<int>(std::lround(clamp01(hit_uv.Y) * texture_extent)),
                    0,
                    std::max(0, texture_size - 1)));
                const auto uv_key = (static_cast<std::uint64_t>(u_pixel) << 32) | static_cast<std::uint64_t>(v_pixel);
                if (!seen_uv.insert(uv_key).second)
                {
                    ++out.duplicate_uv;
                    continue;
                }
                ...
                sample.source_candidate = true;
                sample.facing_dot = -1.0;
                sample.u = clamp01(hit_uv.X);
                sample.v = clamp01(hit_uv.Y);
                sample.barycentric_a = 1.0;
                sample.barycentric_b = 0.0;
                sample.barycentric_c = 0.0;
                sample.world_position = hit_world;
                sample.local_position = mesh_first_transform_inverse_apply_point(component_to_world, hit_world);
                sample.uv_island = -1;
                sample.dominant_bone = -1;
                sample.body_region = "screen_hittest";
                sample.unsafe = false;
                if (profile && runtime_triangles)
                {
                    int resolved_triangle = -1;
                    double resolved_a = 1.0;
                    double resolved_b = 0.0;
                    double resolved_c = 0.0;
                    double resolved_world_delta = std::numeric_limits<double>::infinity();
                    if (mesh_first_lookup_profile_triangle_at_uv(*profile,
                                                                 *runtime_triangles,
                                                                 sample.u,
                                                                 sample.v,
                                                                 hit_world,
                                                                 ...
                                                                 resolved_b,
                                                                 resolved_c,
                                                                 resolved_world_delta))
                    {
                        sample.triangle_index = resolved_triangle;
                        sample.barycentric_a = resolved_a;
                        sample.barycentric_b = resolved_b;
                        sample.barycentric_c = resolved_c;
                        const auto& resolved_runtime_triangle = (*runtime_triangles)[static_cast<std::size_t>(resolved_triangle)];
                        sample.local_position = sdk_vec_add(
                            sdk_vec_add(sdk_vec_mul(resolved_runtime_triangle.local[0], resolved_a),
                                        sdk_vec_mul(resolved_runtime_triangle.local[1], resolved_b)),
                            sdk_vec_mul(resolved_runtime_triangle.local[2], resolved_c));
                        if (static_cast<std::size_t>(resolved_triangle) < profile->triangles.size())
                        {
                            const auto& resolved_meta = profile->triangles[static_cast<std::size_t>(resolved_triangle)];
                            sample.uv_island = resolved_meta.uv_island;
                            sample.dominant_bone = resolved_meta.dominant_bone;
                            sample.body_region = resolved_meta.body_region;
                        }
                        ++out.uv_triangle_lookup_resolved;
                        uv_triangle_world_delta_sum += resolved_world_delta;
                        out.uv_triangle_world_delta_max_cm = std::max(out.uv_triangle_world_delta_max_cm,
                                                                       resolved_world_delta);
                    }
                    else
                    {
                        ++out.uv_triangle_lookup_unresolved;
                    }
                }
        ...
        out.projection_delta_avg_px = out.hits > 0 ? projection_delta_sum / static_cast<double>(out.hits) : 0.0;
        out.uv_triangle_world_delta_avg_cm = out.uv_triangle_lookup_resolved > 0
                                                 ? uv_triangle_world_delta_sum /
                                                       static_cast<double>(out.uv_triangle_lookup_resolved)
                                                 : 0.0;
        out.ok = out.hits >= std::max(1, min_hits);
        out.failure = out.ok ? "ok" : (out.process_ok <= 0 ? "screen_hittest_process_event_failed"
                                                            : "screen_hittest_insufficient_hits");
        return out;
    }
                                  ...
                                  MeshFirstPlanStats& stats) -> void
    {
        ...
        for (std::size_t sample_index = 0; sample_index < samples.size(); ++sample_index)
        {
            ...
            if (visible_direct_only)
            {
                ...
                sample.direct_capture_color = true;
                sample.unsafe = false;
                                             ...
                                             std::string& metadata) -> void
    {
        ...
        for (const auto& sample : samples)
        {
            const bool enabled = (sample.region == MeshFirstRegion::Front && enable_front) ||
                                 (sample.region == MeshFirstRegion::Side && enable_side) ||
                                 (sample.region == MeshFirstRegion::Back && enable_back);
            if (!enabled || sample.unsafe || (visible_direct_only && !sample.direct_capture_color))
            {
                continue;
            }
            draw(color_rgb,
                                         ...
                                         const std::shared_ptr<QueuedPaintJob>& queued_job) -> std::string
    {
        ...
        const bool tuning_visible_direct_only = json_bool_field(request, "visible_direct_only", false);
        const bool tuning_strict_accuracy = json_bool_field(request, "strict_accuracy", false);
        const bool tuning_deduplicate_uv = json_bool_field(request, "deduplicate_uv", tuning_strict_accuracy);
        const bool tuning_screen_hittest_refine = json_bool_field(request, "screen_hittest_refine", false);
        const int tuning_screen_hittest_cell_px = json_int_field(request, "screen_hittest_cell_px", 4, 1, 32);
        const int tuning_screen_hittest_max_samples = json_int_field(request, "screen_hittest_max_samples", 12000, 64, 50000);
        const int tuning_screen_hittest_min_samples = json_int_field(request, "screen_hittest_min_samples", 128, 1, 100000);
        const double tuning_screen_hittest_max_projection_delta_px = clamp_range(
            json_number_field(request, "screen_hittest_max_projection_delta_px", 2.0), 0.1, 16.0);
        const bool tuning_hide_all_character_meshes = json_bool_field(request, "hide_all_character_meshes", true);
        const int tuning_capture_settle_ms = json_int_field(request, "capture_settle_ms", 48, 0, 500);
        const auto tuning_capture_source_name = lower_copy(json_string_field(request, "capture_source", "base_color"));
        const auto tuning_capture_source = tuning_capture_source_name == "final_color_ldr"
                                               ? sdk::ESceneCaptureSource::FinalColorLDR
                                               : sdk::ESceneCaptureSource::BaseColor;
        const int tuning_min_direct_samples = json_int_field(request, "min_direct_samples", 64, 1, 100000);
        const double tuning_max_projection_error_px = clamp_range(json_number_field(request, "max_projection_error_px", 1.0), 0.05, 16.0);
        const double tuning_camera_stability_translation_cm = clamp_range(json_number_field(request, "camera_stability_translation_cm", 5.0), 0.1, 100.0);
        const double tuning_camera_stability_angle_deg = clamp_range(json_number_field(request, "camera_stability_angle_deg", 1.5), 0.1, 30.0);
        const double tuning_component_stability_translation_cm = clamp_range(json_number_field(request, "component_stability_translation_cm", 5.0), 0.1, 100.0);
        const double tuning_component_stability_angle_deg = clamp_range(json_number_field(request, "component_stability_angle_deg", 2.0), 0.1, 45.0);
        const double tuning_pose_stability_avg_cm = clamp_range(json_number_field(request, "pose_stability_avg_cm", 2.0), 0.01, 100.0);
        const double tuning_pose_stability_max_cm = clamp_range(json_number_field(request, "pose_stability_max_cm", 8.0), 0.01, 300.0);
        ...
        const bool tuning_adaptive_uv_brush = json_bool_field(request, "adaptive_uv_brush", false);
        const double tuning_adaptive_uv_brush_max_radius_texels = clamp_range(
            json_number_field(request, "adaptive_uv_brush_max_radius_texels", 14.0), 2.0, 32.0);
        const double tuning_adaptive_uv_brush_neighbor_scale = clamp_range(
            json_number_field(request, "adaptive_uv_brush_neighbor_scale", 0.58), 0.50, 0.90);
        const double tuning_adaptive_uv_brush_min_coverage = clamp_range(
            json_number_field(request, "adaptive_uv_brush_min_coverage", 0.90), 0.50, 1.0);
        ...
        metadata += ",\"runtime_hit_test_used\":" + std::string(json_bool(tuning_screen_hittest_refine));
        ...
        metadata += ",\"visible_direct_only\":" + std::string(json_bool(tuning_visible_direct_only));
        metadata += ",\"strict_accuracy\":" + std::string(json_bool(tuning_strict_accuracy));
        metadata += ",\"deduplicate_uv\":" + std::string(json_bool(tuning_deduplicate_uv));
        metadata += ",\"screen_hittest_refine\":" + std::string(json_bool(tuning_screen_hittest_refine));
        metadata += ",\"screen_hittest_cell_px\":" + std::to_string(tuning_screen_hittest_cell_px);
        metadata += ",\"screen_hittest_max_samples\":" + std::to_string(tuning_screen_hittest_max_samples);
        metadata += ",\"screen_hittest_min_samples\":" + std::to_string(tuning_screen_hittest_min_samples);
        metadata += ",\"screen_hittest_max_projection_delta_px\":" + std::to_string(tuning_screen_hittest_max_projection_delta_px);
        metadata += ",\"hide_all_character_meshes\":" + std::string(json_bool(tuning_hide_all_character_meshes));
        metadata += ",\"capture_settle_ms\":" + std::to_string(tuning_capture_settle_ms);
        metadata += ",\"capture_source\":\"" + json_escape(tuning_capture_source_name) + "\"";
        metadata += ",\"min_direct_samples\":" + std::to_string(tuning_min_direct_samples);
        metadata += ",\"max_projection_error_px\":" + std::to_string(tuning_max_projection_error_px);
        metadata += ",\"camera_stability_translation_cm\":" + std::to_string(tuning_camera_stability_translation_cm);
        metadata += ",\"camera_stability_angle_deg\":" + std::to_string(tuning_camera_stability_angle_deg);
        metadata += ",\"component_stability_translation_cm\":" + std::to_string(tuning_component_stability_translation_cm);
        metadata += ",\"component_stability_angle_deg\":" + std::to_string(tuning_component_stability_angle_deg);
        metadata += ",\"pose_stability_avg_cm\":" + std::to_string(tuning_pose_stability_avg_cm);
        metadata += ",\"pose_stability_max_cm\":" + std::to_string(tuning_pose_stability_max_cm);
        metadata += ",\"color_transfer_mode\":\"" +
                    std::string(tuning_visible_direct_only
                                    ? "camera_visible_direct_capture_only"
                                    : "full_surface_projected_transfer") +
                    "\"";
        ...
        metadata += ",\"adaptive_uv_brush\":" + std::string(json_bool(tuning_adaptive_uv_brush));
        metadata += ",\"adaptive_uv_brush_max_radius_texels\":" + std::to_string(tuning_adaptive_uv_brush_max_radius_texels);
        metadata += ",\"adaptive_uv_brush_neighbor_scale\":" + std::to_string(tuning_adaptive_uv_brush_neighbor_scale);
        metadata += ",\"adaptive_uv_brush_min_coverage\":" + std::to_string(tuning_adaptive_uv_brush_min_coverage);
        ...
        if (tuning_strict_accuracy && !tuning_visible_direct_only)
        {
            return response_json(false,
                                 "strict_accuracy_requires_visible_direct",
                                 0,
                                 1,
                                 "strict accuracy requires visible_direct_only so inferred colors cannot be replayed",
                                 metadata + ",\"replay_blocked\":true");
        }
        if (tuning_capture_source_name != "base_color" && tuning_capture_source_name != "final_color_ldr")
        {
            return response_json(false,
                                 "capture_source_invalid",
                                 0,
                                 1,
                                 "capture_source must be base_color or final_color_ldr",
                                 metadata + ",\"replay_blocked\":true");
        }
        if (tuning_strict_accuracy && tuning_screen_hittest_refine &&
            tuning_screen_hittest_min_samples > tuning_screen_hittest_max_samples)
        {
            return response_json(false,
                                 "screen_hittest_tuning_invalid",
                                 0,
                                 1,
                                 "screen hit-test minimum samples exceed the maximum attempt budget",
                                 metadata + ",\"replay_blocked\":true");
        }
        if (tuning_strict_accuracy &&
        ...
        {
            return response_json(false,
                                 "strict_accuracy_rejects_fill",
                                 0,
                                 1,
                                 "strict accuracy rejects fill regions because their colors are not direct camera samples",
                                 metadata + ",\"replay_blocked\":true");
        }
        ...
        bool runtime_triangle_pose_reconstruction_attempted = false;
        bool runtime_triangle_pose_reconstruction_used = false;
        std::string runtime_triangle_pose_reconstruction_failure{"not_needed"};
        if (!runtime_triangle_cache.ok)
        {
            runtime_triangle_pose_reconstruction_attempted = true;
            auto reconstructed = mesh_first_reconstruct_profile_pose_triangles(profile,
                                                                                skinned_component_positions,
                                                                                skinned_world_positions);
            runtime_triangle_pose_reconstruction_failure = reconstructed.failure;
            if (skeletal_skin_available && reconstructed.ok)
            {
                runtime_triangle_cache = std::move(reconstructed);
                runtime_triangle_cache_mode = "profile_pose_reconstructed";
                runtime_triangle_pose_reconstruction_used = true;
                runtime_triangle_pose_reconstruction_failure.clear();
                runtime_coordinate_post_warm = runtime_coordinate_probe(runtime_triangle_cache);
            }
            else if (!skeletal_skin_available)
            {
                runtime_triangle_pose_reconstruction_failure =
                    skin_failure.empty() ? "profile_pose_reconstruction_skin_unavailable" : skin_failure;
            }
        }
        ...
        metadata += ",\"runtime_triangle_pose_reconstruction_attempted\":" + std::string(json_bool(runtime_triangle_pose_reconstruction_attempted));
        metadata += ",\"runtime_triangle_pose_reconstruction_used\":" + std::string(json_bool(runtime_triangle_pose_reconstruction_used));
        metadata += ",\"runtime_triangle_pose_reconstruction_failure\":\"" + json_escape(runtime_triangle_pose_reconstruction_failure) + "\"";
        ...
        const bool runtime_private_cache_used =
            runtime_triangle_cache.ok && runtime_triangle_cache_mode == "profile_verified";
        const bool runtime_uses_profile_component_world =
            profile_available &&
            (runtime_private_cache_used || runtime_triangle_pose_reconstruction_used);
        metadata += ",\"runtime_triangle_private_cache_used\":" + std::string(json_bool(runtime_private_cache_used));
        metadata += ",\"planner_position_source\":\"" +
                    std::string(runtime_triangle_pose_reconstruction_used
                                    ? "mesh_profile_trusted_pose_reconstructed"
                                    : (runtime_uses_profile_component_world
        ...
        metadata += ",\"pose_used_for_replay_anchor\":" + std::string(json_bool(runtime_private_cache_used));
        metadata += ",\"skeletal_pose_used_for_projection\":" + std::string(json_bool(runtime_triangle_pose_reconstruction_used));
        metadata += ",\"runtime_triangle_cache_pose_used\":" + std::string(json_bool(runtime_private_cache_used));
        if (!runtime_triangle_cache.ok)
        {
            return response_json(false,
                                 ...
                                 0,
                                 1,
                                 "RuntimePaintable cached current triangles and trusted-pose reconstruction are unavailable; mesh-first paint cannot plan safely",
                                 metadata + ",\"replay_blocked\":true");
        }
        ...
        const auto raw_runtime_projection_selection =
            mesh_first_select_runtime_triangle_projection_coordinates(ref,
                                                                     ...
                                                                     camera_direction,
                                                                     viewport,
                                                                     runtime_triangle_pose_reconstruction_used
                                                                         ? "profile_pose_reconstructed_world"
        ...
        metadata += ",\"runtime_triangle_projection_active_source\":\"" +
                    std::string(runtime_triangle_pose_reconstruction_used
                                    ? "profile_pose_reconstructed_local_component_world"
        ...
        const auto profile_winding = profile_available
                                         ? mesh_first_analyze_profile_winding(profile)
                                         : MeshFirstProfileWindingAnalysis{};
        const bool invert_camera_facing_winding =
            runtime_triangle_pose_reconstruction_used &&
            (profile_winding.ok ? profile_winding.inverted : true);
        metadata += ",\"mesh_region_threshold\":0.350000";
        metadata += ",\"mesh_region_threshold_source\":\"fixed_mesh_local_normal\"";
        metadata += ",\"profile_winding_analysis_ok\":" + std::string(json_bool(profile_winding.ok));
        metadata += ",\"profile_winding_outward\":" + std::to_string(profile_winding.outward);
        metadata += ",\"profile_winding_inward\":" + std::to_string(profile_winding.inward);
        metadata += ",\"profile_winding_ambiguous\":" + std::to_string(profile_winding.ambiguous);
        metadata += ",\"profile_winding_confidence\":" + std::to_string(profile_winding.confidence);
        metadata += ",\"profile_winding_failure\":\"" + json_escape(profile_winding.failure) + "\"";
        metadata += ",\"planner_camera_facing_winding\":\"" +
                    std::string(runtime_triangle_pose_reconstruction_used
                                    ? (profile_winding.ok
                                           ? (invert_camera_facing_winding
                                                  ? "profile_pose_reconstructed_auto_inverted"
                                                  : "profile_pose_reconstructed_auto_native")
                                           : "profile_pose_reconstructed_legacy_inverted_fallback")
                                    : "runtime_native") +
                    "\"";
        if (tuning_strict_accuracy && runtime_triangle_pose_reconstruction_used && !profile_winding.ok)
        {
            return response_json(false,
                                 "profile_winding_untrusted",
                                 0,
                                 1,
                                 "strict accuracy could not determine the mesh triangle winding with sufficient confidence",
                                 metadata + ",\"replay_blocked\":true");
        }
        ...
        MeshFirstScreenHitRefineResult screen_hit_refine{};
        bool screen_hit_refine_used = false;
        if (any_paint_region && tuning_screen_hittest_refine)
        {
            write_bridge_progress("screen_hittest_refine",
                                  "Resolving visible runtime UVs from screen hit tests",
                                  3,
                                  5,
                                  0.0,
                                  "\"seed_samples\":" + std::to_string(plan_samples.size()));
            screen_hit_refine = mesh_first_refine_plan_with_uncached_screen_hits(
                ref,
                ctx,
                selected_mesh.mesh,
                viewport,
                ...
                plan_samples,
                active_texture_size,
                tuning_screen_hittest_cell_px,
                tuning_screen_hittest_max_samples,
                std::max(tuning_min_direct_samples, tuning_screen_hittest_min_samples),
                tuning_screen_hittest_max_projection_delta_px);
            metadata += ",\"screen_hittest_ok\":" + std::string(json_bool(screen_hit_refine.ok));
            metadata += ",\"screen_hittest_failure\":\"" + json_escape(screen_hit_refine.failure) + "\"";
            metadata += ",\"screen_hittest_function\":\"" + hex_address(screen_hit_refine.function) + "\"";
            metadata += ",\"screen_hittest_seed_projected\":" + std::to_string(screen_hit_refine.seed_projected);
            metadata += ",\"screen_hittest_bbox_min_x\":" + std::to_string(screen_hit_refine.bbox_min_x);
            metadata += ",\"screen_hittest_bbox_min_y\":" + std::to_string(screen_hit_refine.bbox_min_y);
            metadata += ",\"screen_hittest_bbox_max_x\":" + std::to_string(screen_hit_refine.bbox_max_x);
            metadata += ",\"screen_hittest_bbox_max_y\":" + std::to_string(screen_hit_refine.bbox_max_y);
            metadata += ",\"screen_hittest_requested_cell_px\":" + std::to_string(screen_hit_refine.requested_cell_px);
            metadata += ",\"screen_hittest_effective_cell_px\":" + std::to_string(screen_hit_refine.effective_cell_px);
            metadata += ",\"screen_hittest_attempts\":" + std::to_string(screen_hit_refine.attempts);
            metadata += ",\"screen_hittest_process_ok\":" + std::to_string(screen_hit_refine.process_ok);
            metadata += ",\"screen_hittest_hits\":" + std::to_string(screen_hit_refine.hits);
            metadata += ",\"screen_hittest_invalid_uv\":" + std::to_string(screen_hit_refine.invalid_uv);
            metadata += ",\"screen_hittest_invalid_world\":" + std::to_string(screen_hit_refine.invalid_world);
            metadata += ",\"screen_hittest_projection_rejected\":" + std::to_string(screen_hit_refine.projection_rejected);
            metadata += ",\"screen_hittest_duplicate_uv\":" + std::to_string(screen_hit_refine.duplicate_uv);
            metadata += ",\"screen_hittest_uv_triangle_lookup_resolved\":" + std::to_string(screen_hit_refine.uv_triangle_lookup_resolved);
            metadata += ",\"screen_hittest_uv_triangle_lookup_unresolved\":" + std::to_string(screen_hit_refine.uv_triangle_lookup_unresolved);
            metadata += ",\"screen_hittest_uv_triangle_world_delta_avg_cm\":" + std::to_string(screen_hit_refine.uv_triangle_world_delta_avg_cm);
            metadata += ",\"screen_hittest_uv_triangle_world_delta_max_cm\":" + std::to_string(screen_hit_refine.uv_triangle_world_delta_max_cm);
            const double screen_hittest_uv_triangle_lookup_ratio = screen_hit_refine.hits > 0
                                                                        ? static_cast<double>(screen_hit_refine.uv_triangle_lookup_resolved) /
                                                                              static_cast<double>(screen_hit_refine.hits)
                                                                        : 0.0;
            metadata += ",\"screen_hittest_uv_triangle_lookup_ratio\":" + std::to_string(screen_hittest_uv_triangle_lookup_ratio);
            metadata += ",\"screen_hittest_projection_delta_avg_px\":" + std::to_string(screen_hit_refine.projection_delta_avg_px);
            metadata += ",\"screen_hittest_projection_delta_max_px\":" + std::to_string(screen_hit_refine.projection_delta_max_px);
            if (!screen_hit_refine.ok)
            {
                if (tuning_strict_accuracy)
                {
                    return response_json(false,
                                         "screen_hittest_refine_failed",
                                         0,
                                         1,
                                         "strict accuracy could not map enough visible screen pixels to exact runtime UVs",
                                         metadata + ",\"replay_blocked\":true");
                }
            }
            else if (tuning_strict_accuracy && profile_available &&
                     screen_hittest_uv_triangle_lookup_ratio < 0.85)
            {
                return response_json(false,
                                     "screen_hittest_uv_island_lookup_failed",
                                     0,
                                     1,
                                     "strict accuracy could not assign enough exact screen hits to safe UV islands",
                                     metadata + ",\"replay_blocked\":true");
            }
            else
            {
                plan_samples = std::move(screen_hit_refine.samples);
                screen_hit_refine_used = true;
                plan_stats.total_samples = static_cast<int>(plan_samples.size());
                plan_stats.source_samples = plan_stats.total_samples;
                plan_stats.front_samples = plan_stats.total_samples;
                plan_stats.side_samples = 0;
                plan_stats.back_samples = 0;
                plan_stats.enabled_samples = 0;
            }
        }
        metadata += ",\"screen_hittest_refine_used\":" + std::string(json_bool(screen_hit_refine_used));
        SdkNativeFrontSampleResult native_front{};
        native_front.mesh = selected_mesh.mesh;
        native_front.capture_source = tuning_capture_source;
        native_front.capture_source_name = tuning_capture_source_name;
        native_front.capture_settle_ms = tuning_capture_settle_ms;
        if (tuning_hide_all_character_meshes)
        {
            for (const auto& candidate : mesh_candidates)
            {
                if (live_uobject(candidate.mesh))
                {
                    native_front.meshes_to_hide.push_back(candidate.mesh);
                }
            }
        }
        else
        {
            native_front.meshes_to_hide.push_back(selected_mesh.mesh);
        }
        ...
        if (any_paint_region)
        {
            ...
            if (tuning_strict_accuracy)
            {
                const bool strict_capture_ok =
                    capture.capture_pixels_available &&
                    capture.hide_component_called &&
                    capture.missing_color == 0 &&
                    capture.project_success > 0 &&
                    std::isfinite(capture.project_delta_max_px) &&
                    capture.project_delta_max_px <= tuning_max_projection_error_px;
                metadata += ",\"strict_capture_ok\":" + std::string(json_bool(strict_capture_ok));
                metadata += ",\"strict_capture_hidden_mesh_confirmed\":" + std::string(json_bool(capture.hide_component_called));
                if (!strict_capture_ok)
                {
                    return response_json(false,
                                         "strict_capture_validation_failed",
                                         0,
                                         1,
                                         "strict accuracy rejected the capture because the hidden-mesh, pixel, or projection checks failed",
                                         metadata + ",\"replay_blocked\":true");
                }

                const auto stable_ray = sdk_deproject_screen_position(ref,
                                                                      ctx,
                                                                      static_cast<double>(viewport.width) * 0.5,
                                                                      static_cast<double>(viewport.height) * 0.5);
                const double camera_translation_delta = stable_ray.ok
                                                            ? sdk_vec_len(sdk_vec_sub(stable_ray.location, center_ray.location))
                                                            : std::numeric_limits<double>::infinity();
                const auto stable_direction = stable_ray.ok ? sdk_vec_normalize(stable_ray.direction) : sdk::FVector{};
                const double direction_dot = stable_ray.ok
                                                 ? clamp_range(sdk_vec_dot(camera_direction, stable_direction), -1.0, 1.0)
                                                 : -1.0;
                const double camera_angle_delta = stable_ray.ok
                                                      ? std::acos(direction_dot) * 180.0 / 3.14159265358979323846
                                                      : std::numeric_limits<double>::infinity();
                metadata += ",\"camera_stability_ray_ok\":" + std::string(json_bool(stable_ray.ok));
                metadata += ",\"camera_stability_translation_delta_cm\":" + std::to_string(camera_translation_delta);
                metadata += ",\"camera_stability_angle_delta_deg\":" + std::to_string(camera_angle_delta);

                sdk::FTransform stable_component_to_world{};
                std::string stable_component_source{};
                const bool stable_component_ok = mesh_first_resolve_component_to_world(ref,
                                                                                       selected_mesh.mesh,
                                                                                       ctx.body_world_position,
                                                                                       stable_component_to_world,
                                                                                       stable_component_source);
                const double component_translation_delta = stable_component_ok
                                                               ? sdk_vec_len(sdk_vec_sub(stable_component_to_world.Translation,
                                                                                         component_to_world.Translation))
                                                               : std::numeric_limits<double>::infinity();
                const auto component_q0 = mesh_first_quat_normalize(component_to_world.Rotation);
                const auto component_q1 = mesh_first_quat_normalize(stable_component_to_world.Rotation);
                const double component_q_dot = stable_component_ok
                                                   ? std::abs(component_q0.X * component_q1.X +
                                                              component_q0.Y * component_q1.Y +
                                                              component_q0.Z * component_q1.Z +
                                                              component_q0.W * component_q1.W)
                                                   : 0.0;
                const double component_angle_delta = stable_component_ok
                                                         ? 2.0 * std::acos(clamp_range(component_q_dot, 0.0, 1.0)) *
                                                               180.0 / 3.14159265358979323846
                                                         : std::numeric_limits<double>::infinity();
                metadata += ",\"component_stability_ok\":" + std::string(json_bool(stable_component_ok));
                metadata += ",\"component_stability_source\":\"" + json_escape(stable_component_source) + "\"";
                metadata += ",\"component_stability_translation_delta_cm\":" + std::to_string(component_translation_delta);
                metadata += ",\"component_stability_angle_delta_deg\":" + std::to_string(component_angle_delta);

                bool stable_pose_ok = !runtime_triangle_pose_reconstruction_used;
                double pose_stability_avg_delta = 0.0;
                double pose_stability_max_delta = 0.0;
                std::string pose_stability_failure = runtime_triangle_pose_reconstruction_used ? "not_run" : "not_required_native_cache";
                if (runtime_triangle_pose_reconstruction_used && stable_component_ok)
                {
                    auto stable_pose = sdk_resolve_skinned_pose(ref, selected_mesh.mesh, profile.bone_count);
                    if (stable_pose.ok && mesh_first_validate_pose_for_profile(profile, stable_pose))
                    {
                        std::vector<sdk::FVector> stable_component_positions{};
                        std::vector<sdk::FVector> stable_world_positions{};
                        std::string stable_skin_failure{};
                        if (mesh_first_skin_vertices(profile,
                                                     stable_pose,
                                                     stable_component_to_world,
                                                     stable_component_positions,
                                                     stable_world_positions,
                                                     stable_skin_failure) &&
                            stable_component_positions.size() == skinned_component_positions.size() &&
                            !stable_component_positions.empty())
                        {
                            double delta_sum = 0.0;
                            for (std::size_t i = 0; i < stable_component_positions.size(); ++i)
                            {
                                const double delta = sdk_vec_len(sdk_vec_sub(stable_component_positions[i],
                                                                             skinned_component_positions[i]));
                                delta_sum += delta;
                                pose_stability_max_delta = std::max(pose_stability_max_delta, delta);
                            }
                            pose_stability_avg_delta = delta_sum / static_cast<double>(stable_component_positions.size());
                            stable_pose_ok = std::isfinite(pose_stability_avg_delta) &&
                                             std::isfinite(pose_stability_max_delta) &&
                                             pose_stability_avg_delta <= tuning_pose_stability_avg_cm &&
                                             pose_stability_max_delta <= tuning_pose_stability_max_cm;
                            pose_stability_failure = stable_pose_ok ? "ok" : "pose_changed_during_capture";
                        }
                        else
                        {
                            pose_stability_failure = stable_skin_failure.empty()
                                                         ? "stable_pose_skin_failed"
                                                         : stable_skin_failure;
                        }
                    }
                    else
                    {
                        pose_stability_failure = stable_pose.validation_failure.empty()
                                                     ? "stable_pose_unavailable"
                                                     : stable_pose.validation_failure;
                    }
                }
                metadata += ",\"pose_stability_ok\":" + std::string(json_bool(stable_pose_ok));
                metadata += ",\"pose_stability_avg_delta_cm\":" + std::to_string(pose_stability_avg_delta);
                metadata += ",\"pose_stability_max_delta_cm\":" + std::to_string(pose_stability_max_delta);
                metadata += ",\"pose_stability_failure\":\"" + json_escape(pose_stability_failure) + "\"";

                const bool capture_state_stable =
                    stable_ray.ok &&
                    camera_translation_delta <= tuning_camera_stability_translation_cm &&
                    camera_angle_delta <= tuning_camera_stability_angle_deg &&
                    stable_component_ok &&
                    component_translation_delta <= tuning_component_stability_translation_cm &&
                    component_angle_delta <= tuning_component_stability_angle_deg &&
                    stable_pose_ok;
                metadata += ",\"capture_state_stable\":" + std::string(json_bool(capture_state_stable));
                if (!capture_state_stable)
                {
                    return response_json(false,
                                         "capture_state_changed",
                                         0,
                                         1,
                                         "camera, character component, or skeletal pose moved while the background was being captured; hold still and retry",
                                         metadata + ",\"replay_blocked\":true");
                }
            }
            ...
            mesh_first_assign_colors(profile_available ? &profile : nullptr,
                                     ...
                                     enable_side,
                                     enable_back,
                                     tuning_visible_direct_only,
                                     tuning_side_source_max_uv,
                                     plan_stats);

            if (tuning_strict_accuracy)
            {
                const bool direct_plan_ok =
                    plan_stats.source_projection_assignments == 0 &&
                    plan_stats.source_direct_assignments >= tuning_min_direct_samples &&
                    plan_stats.enabled_samples == plan_stats.source_direct_assignments;
                metadata += ",\"strict_direct_plan_ok\":" + std::string(json_bool(direct_plan_ok));
                if (!direct_plan_ok)
                {
                    metadata += ",";
                    metadata += mesh_first_plan_stats_metadata(plan_stats);
                    return response_json(false,
                                         "strict_direct_plan_failed",
                                         0,
                                         1,
                                         "strict accuracy found inferred colors, too few direct samples, or an inconsistent direct-sample count",
                                         metadata + ",\"replay_blocked\":true");
                }
            }
        }
        ...
        metadata += ",\"source_distance_policy\":\"" +
                    std::string(tuning_visible_direct_only
                                    ? "camera_visible_direct_capture_pixel_only"
                                    : (profile_available ? "side_visible_pose_component_nearest_front_back_projection_pixel"
        ...
        safe_copy(&brush,
                  ...
                  sizeof(brush));
        // Do not inherit a user-selected/stale patterned brush texture.  Auto
        // paint needs a solid footprint or a dense set of correct UV samples is
        // rendered as a dotted mask instead of continuous camouflage.
        brush.BrushTexture = nullptr;
        brush.Hardness = 1.0f;
        brush.Opacity = 1.0f;
        const double stroke_radius_texels = tuning_stroke_size_texels * 0.5;
        ...
        const bool use_mesh_anchors = runtime_private_cache_used && !screen_hit_refine_used;
        const std::string replay_anchor_policy =
            use_mesh_anchors
                ? "profile_verified_triangle_anchor"
                : (screen_hit_refine_used
                       ? "uv_only_uncached_screen_hittest"
                : (runtime_triangle_pose_reconstruction_used
                       ? "uv_only_profile_pose_reconstructed"
        ...
        int replay_non_direct_skipped = 0;
        int replay_uv_candidates = 0;
        int replay_uv_unique = 0;
        int replay_uv_duplicates = 0;
        int replay_uv_color_conflicts = 0;
        int replay_uv_duplicate_skipped = 0;
        auto replay_uv_key = [&](const MeshFirstPlanSample& sample) -> std::uint64_t {
            const auto texture_extent = static_cast<double>(std::max(1, active_texture_size - 1));
            const auto u_pixel = static_cast<std::uint32_t>(std::clamp<int>(
                static_cast<int>(std::lround(clamp01(sample.u) * texture_extent)),
                0,
                std::max(0, active_texture_size - 1)));
            const auto v_pixel = static_cast<std::uint32_t>(std::clamp<int>(
                static_cast<int>(std::lround(clamp01(sample.v) * texture_extent)),
                0,
                std::max(0, active_texture_size - 1)));
            return (static_cast<std::uint64_t>(u_pixel) << 32) | static_cast<std::uint64_t>(v_pixel);
        };
        std::unordered_map<std::uint64_t, std::size_t> replay_uv_choice{};
        if (tuning_deduplicate_uv)
        {
            replay_uv_choice.reserve(plan_samples.size());
            const auto capture_forward = sdk_vec_normalize(capture.capture_direction);
            for (std::size_t sample_index = 0; sample_index < plan_samples.size(); ++sample_index)
            {
                const auto& sample = plan_samples[sample_index];
                const auto region_mode = mesh_first_region_mode_for_sample(sample.region,
                                                                           front_region_mode,
                                                                           side_region_mode,
                                                                           back_region_mode);
                if (region_mode == MeshFirstRegionMode::Skip || sample.unsafe ||
                    (tuning_visible_direct_only && !sample.direct_capture_color))
                {
                    continue;
                }
                ++replay_uv_candidates;
                const auto key = replay_uv_key(sample);
                const auto existing = replay_uv_choice.find(key);
                if (existing == replay_uv_choice.end())
                {
                    replay_uv_choice.emplace(key, sample_index);
                    continue;
                }
                ++replay_uv_duplicates;
                const auto& previous = plan_samples[existing->second];
                const double color_delta = std::max({std::abs(sample.r - previous.r),
                                                     std::abs(sample.g - previous.g),
                                                     std::abs(sample.b - previous.b)});
                if (color_delta > 0.08)
                {
                    ++replay_uv_color_conflicts;
                }
                const double previous_depth = sdk_vec_dot(sdk_vec_sub(previous.world_position, capture.capture_location), capture_forward);
                const double candidate_depth = sdk_vec_dot(sdk_vec_sub(sample.world_position, capture.capture_location), capture_forward);
                if (std::isfinite(candidate_depth) && candidate_depth > 0.0 &&
                    (!std::isfinite(previous_depth) || previous_depth <= 0.0 || candidate_depth < previous_depth))
                {
                    existing->second = sample_index;
                }
            }
            replay_uv_unique = static_cast<int>(replay_uv_choice.size());
        }
        metadata += ",\"replay_uv_dedup_enabled\":" + std::string(json_bool(tuning_deduplicate_uv));
        metadata += ",\"replay_uv_candidates\":" + std::to_string(replay_uv_candidates);
        metadata += ",\"replay_uv_unique\":" + std::to_string(replay_uv_unique);
        metadata += ",\"replay_uv_duplicates\":" + std::to_string(replay_uv_duplicates);
        metadata += ",\"replay_uv_color_conflicts\":" + std::to_string(replay_uv_color_conflicts);
        if (tuning_strict_accuracy && replay_uv_color_conflicts > 0)
        {
            return response_json(false,
                                 "strict_uv_overlap_conflict",
                                 0,
                                 1,
                                 "strict accuracy found the same UV texel mapped to visibly different direct colors; replay was blocked instead of overwriting it",
                                 metadata + ",\"replay_blocked\":true");
        }

        std::vector<double> adaptive_radius_texels(plan_samples.size(), stroke_radius_texels);
        int adaptive_candidates = 0;
        int adaptive_island_groups = 0;
        int adaptive_unresolved_island_candidates = 0;
        int adaptive_neighbor_edges = 0;
        int adaptive_neighbor_edges_covered = 0;
        int adaptive_radius_capped = 0;
        double adaptive_radius_avg = stroke_radius_texels;
        double adaptive_radius_p95 = stroke_radius_texels;
        double adaptive_radius_max = stroke_radius_texels;
        double adaptive_neighbor_coverage_ratio = 1.0;
        if (tuning_adaptive_uv_brush && screen_hit_refine_used)
        {
            struct AdaptiveUvNeighbor
            {
                double distance_texels{std::numeric_limits<double>::infinity()};
                std::size_t sample_index{std::numeric_limits<std::size_t>::max()};
            };
            std::vector<std::array<AdaptiveUvNeighbor, 8>> nearest_by_direction(plan_samples.size());
            std::unordered_map<int, std::vector<std::size_t>> samples_by_island{};
            samples_by_island.reserve(64);
            for (std::size_t sample_index = 0; sample_index < plan_samples.size(); ++sample_index)
            {
                const auto& sample = plan_samples[sample_index];
                const auto region_mode = mesh_first_region_mode_for_sample(sample.region,
                                                                           ...
                                                                           side_region_mode,
                                                                           back_region_mode);
                if (region_mode != MeshFirstRegionMode::Paint || sample.unsafe ||
                    (tuning_visible_direct_only && !sample.direct_capture_color))
                {
                    continue;
                }
                if (tuning_deduplicate_uv)
                {
                    const auto chosen = replay_uv_choice.find(replay_uv_key(sample));
                    if (chosen == replay_uv_choice.end() || chosen->second != sample_index)
                    {
                        continue;
                    }
                }
                ++adaptive_candidates;
                if (sample.uv_island < 0)
                {
                    ++adaptive_unresolved_island_candidates;
                }
                samples_by_island[sample.uv_island].push_back(sample_index);
            }
            adaptive_island_groups = static_cast<int>(samples_by_island.size());
            constexpr double kTwoPi = 6.28318530717958647692;
            const double max_neighbor_distance_texels = tuning_adaptive_uv_brush_max_radius_texels * 2.0;
            auto direction_bin = [&](double du, double dv) -> int {
                double normalized = (std::atan2(dv, du) + 3.14159265358979323846) / kTwoPi;
                int bin = static_cast<int>(std::floor(normalized * 8.0));
                return std::max(0, std::min(7, bin));
            };
            for (const auto& group : samples_by_island)
            {
                const auto& indices = group.second;
                for (std::size_t left_slot = 0; left_slot < indices.size(); ++left_slot)
                {
                    const auto left_index = indices[left_slot];
                    const auto& left = plan_samples[left_index];
                    for (std::size_t right_slot = left_slot + 1; right_slot < indices.size(); ++right_slot)
                    {
                        const auto right_index = indices[right_slot];
                        const auto& right = plan_samples[right_index];
                        const double du = (right.u - left.u) * static_cast<double>(active_texture_size);
                        const double dv = (right.v - left.v) * static_cast<double>(active_texture_size);
                        const double distance = std::sqrt(du * du + dv * dv);
                        if (!std::isfinite(distance) || distance <= 0.25 || distance > max_neighbor_distance_texels)
                        {
                            continue;
                        }
                        const int left_bin = direction_bin(du, dv);
                        const int right_bin = direction_bin(-du, -dv);
                        auto& left_neighbor = nearest_by_direction[left_index][static_cast<std::size_t>(left_bin)];
                        if (distance < left_neighbor.distance_texels)
                        {
                            left_neighbor.distance_texels = distance;
                            left_neighbor.sample_index = right_index;
                        }
                        auto& right_neighbor = nearest_by_direction[right_index][static_cast<std::size_t>(right_bin)];
                        if (distance < right_neighbor.distance_texels)
                        {
                            right_neighbor.distance_texels = distance;
                            right_neighbor.sample_index = left_index;
                        }
                    }
                }
            }

            std::vector<double> selected_radii{};
            selected_radii.reserve(static_cast<std::size_t>(std::max(0, adaptive_candidates)));
            for (const auto& group : samples_by_island)
            {
                for (const auto sample_index : group.second)
                {
                    double radius = stroke_radius_texels;
                    for (const auto& neighbor : nearest_by_direction[sample_index])
                    {
                        if (!std::isfinite(neighbor.distance_texels) ||
                            neighbor.sample_index == std::numeric_limits<std::size_t>::max())
                        {
                            continue;
                        }
                        radius = std::max(radius,
                                          neighbor.distance_texels * tuning_adaptive_uv_brush_neighbor_scale + 0.5);
                    }
                    if (radius > tuning_adaptive_uv_brush_max_radius_texels)
                    {
                        ++adaptive_radius_capped;
                    }
                    radius = clamp_range(radius,
                                         stroke_radius_texels,
                                         tuning_adaptive_uv_brush_max_radius_texels);
                    adaptive_radius_texels[sample_index] = radius;
                    selected_radii.push_back(radius);
                }
            }
            for (const auto& group : samples_by_island)
            {
                for (const auto sample_index : group.second)
                {
                    for (const auto& neighbor : nearest_by_direction[sample_index])
                    {
                        ...
                        ++adaptive_neighbor_edges;
                        const double covered_radius = adaptive_radius_texels[sample_index] +
                                                      adaptive_radius_texels[neighbor.sample_index];
                        if (covered_radius + 0.25 >= neighbor.distance_texels)
                        {
                            ++adaptive_neighbor_edges_covered;
                        }
                    }
                }
            }
            if (!selected_radii.empty())
            {
                std::sort(selected_radii.begin(), selected_radii.end());
                double radius_sum = 0.0;
                for (const auto radius : selected_radii)
                {
                    radius_sum += radius;
                }
                adaptive_radius_avg = radius_sum / static_cast<double>(selected_radii.size());
                const auto p95_index = std::min(selected_radii.size() - 1,
                                                static_cast<std::size_t>(std::floor(
                                                    static_cast<double>(selected_radii.size() - 1) * 0.95)));
                adaptive_radius_p95 = selected_radii[p95_index];
                adaptive_radius_max = selected_radii.back();
            }
            adaptive_neighbor_coverage_ratio = adaptive_neighbor_edges > 0
                                                    ? static_cast<double>(adaptive_neighbor_edges_covered) /
                                                          static_cast<double>(adaptive_neighbor_edges)
                                                    : 0.0;
        }
        metadata += ",\"adaptive_uv_candidates\":" + std::to_string(adaptive_candidates);
        metadata += ",\"adaptive_uv_island_groups\":" + std::to_string(adaptive_island_groups);
        metadata += ",\"adaptive_uv_unresolved_island_candidates\":" + std::to_string(adaptive_unresolved_island_candidates);
        metadata += ",\"adaptive_uv_neighbor_edges\":" + std::to_string(adaptive_neighbor_edges);
        metadata += ",\"adaptive_uv_neighbor_edges_covered\":" + std::to_string(adaptive_neighbor_edges_covered);
        metadata += ",\"adaptive_uv_neighbor_coverage_ratio\":" + std::to_string(adaptive_neighbor_coverage_ratio);
        metadata += ",\"adaptive_uv_radius_capped\":" + std::to_string(adaptive_radius_capped);
        metadata += ",\"adaptive_uv_radius_avg_texels\":" + std::to_string(adaptive_radius_avg);
        metadata += ",\"adaptive_uv_radius_p95_texels\":" + std::to_string(adaptive_radius_p95);
        metadata += ",\"adaptive_uv_radius_max_texels\":" + std::to_string(adaptive_radius_max);
        if (tuning_strict_accuracy && tuning_adaptive_uv_brush && screen_hit_refine_used &&
            (adaptive_neighbor_edges <= 0 ||
             adaptive_neighbor_coverage_ratio < tuning_adaptive_uv_brush_min_coverage))
        {
            return response_json(false,
                                 "adaptive_uv_coverage_failed",
                                 0,
                                 1,
                                 "adaptive UV brush could not cover enough local neighbor gaps safely",
                                 metadata + ",\"replay_blocked\":true");
        }
        ...
        for (const auto target_region : replay_region_order)
        {
            ...
            for (std::size_t sample_index = 0; sample_index < plan_samples.size(); ++sample_index)
            {
                ...
                if (tuning_visible_direct_only && !sample.direct_capture_color)
                {
                    ++replay_non_direct_skipped;
                    continue;
                }
                ...
                auto stroke_brush = fill_mode ? fill_brush : brush;
                if (!fill_mode && tuning_adaptive_uv_brush && sample_index < adaptive_radius_texels.size())
                {
                    stroke_brush.Radius = static_cast<float>(
                        adaptive_radius_texels[sample_index] /
                        static_cast<double>(std::max(1, active_texture_size)));
                }
        ...
        metadata += ",\"replay_non_direct_skipped\":" + std::to_string(replay_non_direct_skipped);
        metadata += ",\"replay_uv_duplicate_skipped\":" + std::to_string(replay_uv_duplicate_skipped);
                                       ...
                                       std::string& failure) -> bool
    {
        ...
        compact.EmissiveColor.R = emissive;
        compact.EmissiveColor.G = emissive;
        compact.EmissiveColor.B = emissive;
        compact.EmissiveColor.A = 255;
    ...
    auto sdk_configure_scene_capture_component_typed(std::uintptr_t capture_component,
                                                      std::uintptr_t render_target,
                                                      double fov_degrees,
                                                      sdk::ESceneCaptureSource capture_source) -> bool
    {
        ...
        __try
        {
            ...
            *reinterpret_cast<std::uint8_t*>(capture_component + sdk::FieldOffsets::SceneCaptureComponent_CaptureSource) =
                static_cast<std::uint8_t>(capture_source);
                                  ...
                                  int target_height) -> SdkFrontCaptureResult
    {
        ...
        out.capture_source = native_front.capture_source_name;
        out.capture_settle_ms = std::max(0, std::min(500, native_front.capture_settle_ms));
        ...
        out.texture_target_written = sdk_configure_scene_capture_component_typed(out.capture_component,
                                                                                  out.render_target,
                                                                                  out.capture_fov,
                                                                                  native_front.capture_source);
        std::vector<std::uintptr_t> meshes_to_hide = native_front.meshes_to_hide;
        if (meshes_to_hide.empty() && native_front.mesh)
        {
            meshes_to_hide.push_back(native_front.mesh);
        }
        std::unordered_set<std::uintptr_t> hidden_meshes{};
        for (const auto mesh : meshes_to_hide)
        {
            if (!live_uobject(mesh) || !hidden_meshes.insert(mesh).second)
            {
                continue;
            }
            ++out.hide_component_requested;
            if (sdk_call_object_param(ref, out.capture_component, "HideComponent", mesh))
            {
                ++out.hide_component_succeeded;
            }
        }
        out.hide_component_called = out.hide_component_requested > 0 &&
                                    out.hide_component_succeeded == out.hide_component_requested;
        ...
        if (sdk_call_no_params(ref, out.capture_component, "CaptureScene"))
        {
            ++out.capture_scene_calls;
        }
        if (out.capture_settle_ms > 0)
        {
            Sleep(static_cast<DWORD>(std::max(1, out.capture_settle_ms / 2)));
        }
        if (sdk_call_no_params(ref, out.capture_component, "CaptureScene"))
        {
            ++out.capture_scene_calls;
        }
        out.capture_scene_called = out.capture_scene_calls == 2;
        ...
        if (out.capture_settle_ms > 0)
        {
            const int remaining_settle = std::max(1, out.capture_settle_ms - out.capture_settle_ms / 2);
            Sleep(static_cast<DWORD>(remaining_settle));
        }
    ...
    auto sdk_capture_metadata(const SdkFrontCaptureResult& capture) -> std::string
    {
        return ",\"front_capture_ok\":" + std::string(json_bool(capture.ok)) +
               ...
               ",\"capture_source\":\"" + json_escape(capture.capture_source) + "\"" +
               ",\"capture_settle_ms\":" + std::to_string(capture.capture_settle_ms) +
               ...
               ",\"front_capture_hide_component_requested\":" + std::to_string(capture.hide_component_requested) +
               ",\"front_capture_hide_component_succeeded\":" + std::to_string(capture.hide_component_succeeded) +
               ",\"front_capture_scene_called\":" + std::string(json_bool(capture.capture_scene_called)) +
               ",\"front_capture_scene_calls\":" + std::to_string(capture.capture_scene_calls) +
    ...
    auto receive_bridge_line(SOCKET client, std::string& line) -> bool
    {
        line.clear();
        line.reserve(65536);
        char buffer[16384]{};
        while (line.size() < MaxRequestBytes)
        {
    ...
    auto send_bridge_text(SOCKET client, const std::string& text) -> bool
    {
        std::size_t offset = 0;
    ...
    auto handle_bridge_client(SOCKET client) -> void
    {
        ...
        std::string hello{};
        if (!receive_bridge_line(client, hello))
        {
            return;
        }
        const bool hello_ok = hello.find("\"type\":\"hello\"") != std::string::npos &&
                              json_int_field(hello, "bootstrap_protocol", 0, 0, 100) == static_cast<int>(BridgeStartProtocol) &&
                              lower_copy(json_string_field(hello, "instance_id", "")) == g_bootstrap_instance_id &&
                              lower_copy(json_string_field(hello, "token", "")) == g_bootstrap_token;
        if (!hello_ok)
        {
            send_bridge_text(client, response_json(false, "hello", 0, 1, "bridge authentication failed"));
            return;
        }
        if (!send_bridge_text(client,
                              response_json(true,
                                            "hello",
                                            0,
                                            0,
                                            "bridge authenticated",
                                            "\"bootstrap_protocol\":1,\"bridge_hash\":\"" +
                                                g_bootstrap_bridge_hash + "\"")))
        {
            return;
        }

        std::string request{};
        if (!receive_bridge_line(client, request))
        {
            return;
        }
        ...
        send_bridge_text(client, response);
    }

    auto bridge_thread(SOCKET listener) -> void
    {
...
extern "C" __declspec(dllexport) DWORD WINAPI BridgeStartV1(void* context)
{
    if (context == nullptr)
    {
    ...
    std::memcpy(&block, context, sizeof(block));

    auto publish_failure = [&](std::uint32_t state, DWORD win32_error, int winsock_error) -> DWORD {
        block.State = state;
        block.Port = 0;
        block.BootstrapProtocol = BridgeStartProtocol;
        block.Win32Error = win32_error;
        block.WinsockError = static_cast<std::uint32_t>(winsock_error);
        std::memcpy(context, &block, sizeof(block));
        return win32_error != ERROR_SUCCESS ? win32_error : ERROR_GEN_FAILURE;
    };

    bool token_nonzero = false;
    for (const auto value : block.Token)
    {
        token_nonzero = token_nonzero || value != 0;
    }
    if (block.Magic != BridgeStartMagic ||
        block.Size != BridgeStartSize ||
        block.Version != BridgeStartProtocol ||
        block.ProcessId != GetCurrentProcessId() ||
        block.BootstrapProtocol != BridgeStartProtocol ||
        !token_nonzero)
    {
        return publish_failure(3, ERROR_INVALID_DATA, 0);
    }

    bool expected = false;
    if (!g_bootstrap_started.compare_exchange_strong(expected, true))
    {
        return publish_failure(5, ERROR_ALREADY_EXISTS, 0);
    }
    ...
    if (startup_error != 0)
    {
        g_bootstrap_started.store(false);
        return publish_failure(6, ERROR_SUCCESS, startup_error);
    }
    ...
    if (listener == INVALID_SOCKET)
    {
        ...
        WSACleanup();
        g_bootstrap_started.store(false);
        return publish_failure(7, ERROR_SUCCESS, error);
    }
    ...
    if (bind(listener, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) == SOCKET_ERROR)
    {
        ...
        WSACleanup();
        g_bootstrap_started.store(false);
        return publish_failure(8, ERROR_SUCCESS, error);
    }

    int addr_size = sizeof(addr);
    if (getsockname(listener, reinterpret_cast<sockaddr*>(&addr), &addr_size) == SOCKET_ERROR ||
        listen(listener, 4) == SOCKET_ERROR)
    {
        ...
        WSACleanup();
        g_bootstrap_started.store(false);
        return publish_failure(9, ERROR_SUCCESS, error);
    }

    const auto port = static_cast<std::uint32_t>(ntohs(addr.sin_port));
    if (port == 0)
    {
        closesocket(listener);
        WSACleanup();
        g_bootstrap_started.store(false);
        return publish_failure(9, ERROR_INVALID_DATA, 0);
    }

    g_bootstrap_instance_id = bytes_to_hex(block.InstanceId, sizeof(block.InstanceId));
    g_bootstrap_token = bytes_to_hex(block.Token, sizeof(block.Token));
    g_bootstrap_bridge_hash = bytes_to_hex(block.BridgeSha256, sizeof(block.BridgeSha256));
    g_running.store(true);

    block.State = 2;
    block.Port = port;
    block.BootstrapProtocol = BridgeStartProtocol;
    block.Win32Error = ERROR_SUCCESS;
    block.WinsockError = 0;
    std::memcpy(context, &block, sizeof(block));

    try
    {
        std::thread(bridge_thread, listener).detach();
    }
    catch (...)
    {
        closesocket(listener);
        WSACleanup();
        g_bootstrap_started.store(false);
        return publish_failure(9, ERROR_NOT_ENOUGH_MEMORY, 0);
    }
