...
class BridgeSession:
    """Connection to one injected bridge instance."""
...
def inject_bridge(game_process=GAME_PROCESS) -> str:
    ...
    bridge_hash_hex = _sha256_file(str(bridge_dll)).hex()

    instance_dir = RUNTIME_DIR / ("bridge-instance-" + instance_guid.hex)
    instance_dir.mkdir(parents=True, exist_ok=True)

    dest_bridge = instance_dir / (
        f"meccha-direct-bridge-v1-"
        f"{bridge_hash_hex}-"
        f"{instance_guid.hex}.dll"
    )
    ...
    try:
        ...
        return f"Could not stage native files: {e}"
    ...
    try:
        port = _choose_bridge_port()
    except OSError as e:
        return f"Could not choose bridge port: {e}"

    port_file = Path(str(dest_bridge) + ".port")

    try:
        port_file.write_text(f"{port}\n", encoding="ascii")
    except OSError as e:
        return f"Could not write bridge port file: {e}"
    ...
    if result.returncode != 0:
        ...
        detail = detail_lines[-1] if detail_lines else detail

        return f"Injector failed: {detail}"

    _bridge_session = BridgeSession(port)
    return ""
...
def _build_tuning(config=None):
    ...
    return {
        ...
        "coverage_step_texels": getattr(config, "coverage_step_texels", 4.0) if config else 4.0,
        ...
        "emissive": getattr(config, "emissive", 0.0) if config else 0.0,
        ...
        "fill_metallic": getattr(config, "fill_metallic", 0.0) if config else 0.0,
        "fill_roughness": getattr(config, "fill_roughness", 1.0) if config else 1.0,
        "fill_emissive": getattr(config, "fill_emissive", 0.0) if config else 0.0,
    }
