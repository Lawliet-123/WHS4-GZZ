...
namespace
{
    ...
    auto sdk_context_metadata(Reflection& ref, const SdkContext& ctx) -> std::string
    {
        return "\"sdk_version\":\"runtime_dynamic_reflection_min\"" +
               ...
               ",\"runtime_paintable_offset\":\"0xb58\"" +
               ...
               ",\"param_schema\":\"FPaintStroke{Uv@0,WorldPosition@16,bHasWorldPosition@40,BrushSettings@104,ChannelData@144,TargetChannel@180};ServerPaintBatch{Batch@0}\"" +
    ...
    auto mesh_first_uncached_screen_hit(Reflection& ref,
                                        const SdkContext& ctx,
                                        std::uintptr_t mesh,
                                        std::uintptr_t hit_test_function,
                                        double screen_x,
                                        double screen_y,
                                        sdk::FScreenSpacePaintResult& result,
                                        std::string& failure) -> bool
    {
        result = {};
        failure.clear();

        if (!hit_test_function)
        {
            failure = "hit_test_function_unavailable";
            return false;
        }
        ...
        if (!live_uobject(ctx.controller))
        {
            failure = "player_controller_unavailable";
            return false;
        }

        if (!std::isfinite(screen_x) || !std::isfinite(screen_y))
        {
            failure = "screen_position_invalid";
            return false;
        }

        sdk::RuntimePaintableComponent_HitTestAtScreenPosition params{};
        params.MeshComponent = reinterpret_cast<void*>(mesh);
        params.ScreenPosition = sdk::FVector2D{screen_x, screen_y};
        params.PlayerController = reinterpret_cast<void*>(ctx.controller);

        params.bUseCachedTriangles = false;

        if (!process_event(ctx.component,
                           hit_test_function,
        ...
        const auto& hit = params.ReturnValue;

        if (!mesh_first_finite_uv(hit.HitUV) ||
            !mesh_first_finite_vector(hit.HitWorldPosition) ||
            !mesh_first_finite_vector(hit.HitNormal) ||
            sdk_vec_len(hit.HitNormal) <= 0.000001)
        {
            failure = "screen_hit_result_invalid";
            return false;
        }
    ...
    struct MeshFirstScreenHitPlanStats
    {
        int coarse_attempts{0};
        int coarse_hits{0};
        int fine_attempts{0};
        int fine_hits{0};
        int duplicate_uv_samples{0};
        int unique_samples{0};
    };

    auto mesh_first_generate_plan_samples_from_screen_hits(
        Reflection& ref,
        const SdkContext& ctx,
        std::uintptr_t mesh,
        const SdkViewportInfo& viewport,
        ...
        std::string& failure) -> bool
    {
        ...
        {
            failure = "screen_hittest_viewport_unavailable";
            return false;
        }

        if (!live_uobject(mesh))
        {
            failure = "screen_hittest_mesh_unavailable";
            return false;
        }

        const auto hit_test_function =
            ref.find_function(ctx.component, "HitTestAtScreenPosition");

        if (!hit_test_function)
        {
            failure = "screen_hittest_function_unavailable";
            return false;
        }

        const double coarse_step =
            clamp_range(coarse_step_pixels, 16.0, 128.0);

        const double fine_step =
            clamp_range(fine_step_pixels, 4.0, coarse_step);

        const double uv_cell_texels =
            clamp_range(coverage_step_texels, 1.0, 64.0);

        const double texture_size_d =
            static_cast<double>(std::max(1, texture_size));

        std::unordered_set<std::uint64_t> occupied_uv_cells{};

        int hit_min_x = viewport.width;
        int hit_min_y = viewport.height;
        int hit_max_x = -1;
        int hit_max_y = -1;

        bool fatal_failure = false;

        const auto add_hit = [&](double screen_x,
                                 double screen_y,
                                 bool coarse_pass) -> void
        {
            if (fatal_failure)
            {
                return;
            ...
            if (coarse_pass)
            {
                ++screen_stats.coarse_attempts;
            }
            else
            {
                ++screen_stats.fine_attempts;
            }

            sdk::FScreenSpacePaintResult hit{};
            std::string hit_failure{};

            const bool hit_ok =
                mesh_first_uncached_screen_hit(ref,
                                               ctx,
                                               mesh,
                                               hit_test_function,
                                               screen_x,
            ...
            if (coarse_pass)
            {
                ++screen_stats.coarse_hits;
            }
            else
            {
                ++screen_stats.fine_hits;
            }
            ...
            hit_max_x = std::max(
                hit_max_x,
                static_cast<int>(std::ceil(screen_x)));

            hit_max_y = std::max(
                hit_max_y,
                static_cast<int>(std::ceil(screen_y)));

            const double u = clamp01(hit.HitUV.X);
            const double v = clamp01(hit.HitUV.Y);

            const int uv_cell_x = static_cast<int>(
                std::floor((u * texture_size_d) / uv_cell_texels));

            const int uv_cell_y = static_cast<int>(
                std::floor((v * texture_size_d) / uv_cell_texels));

            const std::uint64_t uv_key =
                (static_cast<std::uint64_t>(
                     static_cast<std::uint32_t>(uv_cell_x))
                 << 32) |
                static_cast<std::uint64_t>(
                    static_cast<std::uint32_t>(uv_cell_y));

            if (!occupied_uv_cells.insert(uv_key).second)
            {
                ++screen_stats.duplicate_uv_samples;
                return;
            }

            const auto world_normal =
                sdk_vec_normalize(hit.HitNormal);

            const auto inverse_rotation =
                mesh_first_quat_conjugate(
                    mesh_first_quat_normalize(
                        component_to_world.Rotation));

            const auto local_normal =
                sdk_vec_normalize(
                    mesh_first_quat_rotate(
                        inverse_rotation,
                        world_normal));

            const double mesh_facing =
                mesh_first_axis_component(
                    local_normal,
                    region_axis);

            const auto region =
                mesh_facing <= -kMeshFrontThreshold
                    ? MeshFirstRegion::Front
                    : (mesh_facing >= kMeshBackThreshold
                           ? MeshFirstRegion::Back
                           : MeshFirstRegion::Side);
            ...
            sample.barycentric_a = 1.0 / 3.0;
            sample.barycentric_b = 1.0 / 3.0;
            sample.barycentric_c = 1.0 / 3.0;

            sample.world_position = hit.HitWorldPosition;
            sample.local_position =
                mesh_first_transform_inverse_apply_point(
                    component_to_world,
                    hit.HitWorldPosition);

            sample.facing_dot =
                sdk_vec_dot(world_normal, camera_direction);
            ...
            ++mesh_first_sample_region_count(stats, region);
        };

        add_hit(static_cast<double>(viewport.width) * 0.5,
                static_cast<double>(viewport.height) * 0.5,
                true);
        ...
        for (double y = coarse_step * 0.5;
             y < static_cast<double>(viewport.height);
             y += coarse_step)
        {
            for (double x = coarse_step * 0.5;
                 x < static_cast<double>(viewport.width);
                 x += coarse_step)
            {
                add_hit(x, y, true);
        ...
        if (hit_max_x < hit_min_x || hit_max_y < hit_min_y)
        {
            failure = "screen_hittest_no_mesh_hits";
            return false;
        }

        const double fine_min_x =
            std::max(0.0,
                     static_cast<double>(hit_min_x) - coarse_step);

        const double fine_min_y =
            std::max(0.0,
                     static_cast<double>(hit_min_y) - coarse_step);

        const double fine_max_x =
            std::min(static_cast<double>(viewport.width - 1),
                     static_cast<double>(hit_max_x) + coarse_step);

        const double fine_max_y =
            std::min(static_cast<double>(viewport.height - 1),
                     static_cast<double>(hit_max_y) + coarse_step);

        for (double y = fine_min_y;
             y <= fine_max_y;
             y += fine_step)
        {
            for (double x = fine_min_x;
                 x <= fine_max_x;
                 x += fine_step)
            {
                add_hit(x, y, false);
        ...
        screen_stats.unique_samples =
            static_cast<int>(samples.size());

        if (samples.empty())
        {
            failure = "screen_hittest_no_unique_uv_samples";
            return false;
        }
                                         ...
                                         const std::shared_ptr<QueuedPaintJob>& queued_job) -> std::string
    {
        ...
        const bool runtime_cache_needs_warmup =
        ...
        const bool use_screen_hittest_fallback =
            !runtime_triangle_cache.ok;

        if (use_screen_hittest_fallback)
        {
            runtime_triangle_cache_mode =
                "uncached_screen_hittest_fallback";
        }

        metadata += ",\"runtime_hit_test_used\":" +
                    std::string(json_bool(use_screen_hittest_fallback));
        metadata += ",\"screen_hittest_cached_triangles\":false";
        ...
        if (!use_screen_hittest_fallback &&
            runtime_projection_selection.inside_view <= 0)
        {
            return response_json(false,
        ...
        MeshFirstScreenHitPlanStats screen_hit_stats{};
        std::vector<MeshFirstPlanSample> plan_samples{};
        std::string planner_failure{};

        constexpr double screen_hit_coarse_step_pixels = 16.0;
        constexpr double screen_hit_fine_step_pixels = 4.0;
        ...
        {
            planner_ok =
                mesh_first_generate_plan_samples_from_screen_hits(
                    ref,
                    ctx,
                    selected_mesh.mesh,
                    viewport,
                    ...
                    region_axis,
                    active_texture_size,
                    screen_hit_coarse_step_pixels,
                    screen_hit_fine_step_pixels,
        ...
        {
            planner_ok =
                mesh_first_generate_plan_samples_from_runtime_cache(
        ...
        metadata += ",\"mesh_region_threshold_source\":\"" +
                    std::string(use_screen_hittest_fallback
                                    ? "uncached_hittest_world_normal"
                                    : "fixed_mesh_local_normal") +
                    "\"";

        metadata += ",\"screen_hittest_coarse_step_pixels\":" +
                    std::to_string(screen_hit_coarse_step_pixels);
        metadata += ",\"screen_hittest_fine_step_pixels\":" +
                    std::to_string(screen_hit_fine_step_pixels);
        metadata += ",\"screen_hittest_coarse_attempts\":" +
                    std::to_string(screen_hit_stats.coarse_attempts);
        metadata += ",\"screen_hittest_coarse_hits\":" +
                    std::to_string(screen_hit_stats.coarse_hits);
        metadata += ",\"screen_hittest_fine_attempts\":" +
                    std::to_string(screen_hit_stats.fine_attempts);
        metadata += ",\"screen_hittest_fine_hits\":" +
                    std::to_string(screen_hit_stats.fine_hits);
        metadata += ",\"screen_hittest_duplicate_uv_samples\":" +
                    std::to_string(screen_hit_stats.duplicate_uv_samples);
        metadata += ",\"screen_hittest_unique_samples\":" +
                    std::to_string(screen_hit_stats.unique_samples);
        ...
        if (!planner_ok)
        {
            ...
            return response_json(
                false,
                planner_failure.empty()
                    ? "planner_build_failed"
                    : planner_failure.c_str(),
                0,
                1,
                use_screen_hittest_fallback
                    ? "uncached screen HitTest planner could not build a paint plan"
        ...
        native_front.sampling_backend =
            use_screen_hittest_fallback
                ? "uncached_screen_hittest"
                : "mesh_first_camera_facing_mesh_source_projection";

        native_front.min_front_hits =
            use_screen_hittest_fallback
                ? std::max(1, std::min(2048, plan_stats.source_samples))
                : std::max(16, std::min(2048, plan_stats.source_samples));

        native_front.target_front_hits =
            plan_stats.source_samples;

        native_front.hard_attempt_budget =
            use_screen_hittest_fallback
                ? screen_hit_stats.coarse_attempts +
                      screen_hit_stats.fine_attempts
        ...
        brush.Spacing = 1.0f;
        brush.Falloff = sdk::EBrushFalloff::Round;
                                       ...
                                       std::string& failure) -> bool
    {
        ...
        compact.EffectiveSubdivisionLevel = static_cast<std::uint8_t>(std::clamp<std::int32_t>(stroke.EffectiveSubdivisionLevel, 0, 255));
