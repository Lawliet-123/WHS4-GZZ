...
- Separate configurable color for identified Hunters
...
- The Dumper-7 SDK directly declares `SkeletalMesh +0x578`, `SkinnedAsset +0x580`, `LeaderPoseComponent +0x588`, `USkeletalMeshComponent::CachedComponentSpaceTransforms +0x9B8`, `USkeletalMesh::Skeleton +0xF8`, and the `0x60`-byte LWC `FTransform` layout. Named fields are resolved from `UStruct::ChildProperties` at runtime even when the executable fingerprint changes. Blueprint-only `Mesh`, `BodyCapsule`, and `Dead` fields are resolved lazily from the first loaded player Pawn when their class is absent in the home lobby. The 2026-09-19 Steam executable fingerprint is `(0x0ADBC000, 0x74BBB0FB, 0x0A9D9EFB)`. Static disassembly verifies `ComponentToWorld +0x1E0` and the native pose selector/buffers at `+0x5F0/+0x638`; current UHT records verify that the reflected transform-flags byte moved from `+0x1A0` to `+0x240`, so its exact-build fallback is kept in a per-fingerprint override.
- Skeleton lines use the SDK-declared `CachedComponentSpaceTransforms` array first. Every pose payload is accepted only when two adjacent reads match, which rejects a pose being changed mid-copy (including an SDK/native alias). A supported executable may fall back to only the native selector's current buffer; the alternate buffer is never guessed. On a future unknown build, `+0x5F0/+0x638` remain staged locals until one live pose passes reflected mesh identity, selector/header stability, double-read equality, complete `FTransform` validation, and actor-relative world bounds. Only those two pose offsets are then promoted; `ComponentToWorld` remains fingerprint-gated.
- All `pymem` access is read-only. A 30 Hz base worker collects camera, PlayerArray, positions, roles, and optional capsules; a separate latest-job skeleton worker performs every pose read. A blocked or slow pose read therefore cannot stop fresh player positions. The skeleton worker refreshes a fair batch of up to 16 visible actors within a 20 ms launch budget, and pending jobs overwrite older pending jobs instead of forming a backlog. Each new base frame rebases the last verified pose to the current actor-root transform using only in-process matrix math. Transient pose races/read failures retain that verified pose for a bounded 2.5 seconds; actor removal, world changes, or persistent identity/profile/layout failures invalidate it immediately. The actor-to-mesh pointer is revalidated on every slow-path pose read so skin, role, and respawn component swaps cannot pin an obsolete mesh.
- `GameState -> PlayerArray` is accepted only after two identical header-and-payload reads. Read failures and a short same-world collapse from a previously populated array to 0/1 players do not overwrite the last complete frame. A real world/GameState transition still publishes an immediate clear frame. Base player/camera data older than 250 ms is hidden; the independently verified and root-rebased pose layer has its own 2.5-second crowded-lobby tolerance.
...
- The supplied home-lobby Dumper-7 snapshot does not declare the cLeon role fields because that gameplay package was not loaded during the dump. Once a cLeon match is loaded, the reader resolves `HuntersPlayerState`, `LiveSurvivors_PlayerState`, and `MainGamePhase` directly from the live `BP_GameState_cLeon_C` class (`+0x3A0`, `+0x348`, and `+0x418` on the verified build). Two identical reads validate both role arrays. A non-empty Hunter roster supplies roles only in observed main phases 1 and 2, and it can classify an absent remote PlayerState as dead only when the local PlayerState also belongs to one of the two arrays. Phase 0's valid-but-empty arrays deliberately fall back instead of removing every player. Cooked class roles and PlayerState-keyed role caching cover Pawn-class transitions, while the independently reflected `Dead` byte remains the final liveness check. Unreadable death state fails closed for that collection.
- Identified Hunters always use the configurable `Hunter Color` (orange by default), even if the local Pawn is temporarily generic and its role is unavailable. When the local PlayerState is a Hunter, other Hunters are filtered before rendering.
...
`LeaderPoseComponent` follower meshes are deliberately rejected because their leader bone-map path is not implemented. The packaged Paintman and LINK/newpengun profiles remain 28-bone and 40-bone profiles; a live match check is still required after a content update to prove the currently bound player mesh/profile pair.
